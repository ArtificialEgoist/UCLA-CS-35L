...
//    for( int px=0; px<width; ++px )
    for(int px=temp->front; px<temp->back; ++px)
    {
...
int main( int argc, char **argv )
{
    ...
    block* blocks = malloc(nthreads*sizeof(blocks));

    int blockWidth = width/nthreads;

/*
    for(int c=0; c<nthreads; c++)
    {
        blocks[c].front=c*blockWidth;
        blocks[c].back=(c+1)*blockWidth;
    }
*/
...
*/
    ...
    for(int c=0; c<nthreads; c++)
    {
        ...
        aThread=pthread_create(&threads[c], NULL, PixelMethod, (void *)&blocks[c]);
        if(aThread)
        {
